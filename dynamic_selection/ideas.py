...
from dynamic_selection.utils import (ConcreteSelector, MySelector, make_onehot,
                                     restore_parameters)


class AFASelection(nn.Module):
    '''
    ...
    def __init__(self, selector, predictor, mask_layer):
        ...
        self.selector_layer = MySelector()
    ...
    def fit(self,
        ...
        selector_layer = self.selector_layer # Dzung: this is the final layer, we don't need this  
        ...
        for epoch in range(nepochs):
            ...
            for x, y in train_loader:
                ...
                # Dzung: here we don't use max_features because this is not active feature selection
                
                # Dzung: TODO: we cannot do in batch, remove the batch, set = 1
                # Dzung: TODO: finetune alpha and beta
                # Dzung: do in batch is possible, but have to modify the code for the future
                alpha = 0.7
                beta = 0.125
                ...
                while not stop:
                    ...
                    # Get selections
                    prob = selector_layer(logits)
                    
                    m_single = torch.zeros_like(m)
                    m_single[torch.arange(len(m)), torch.argmax(prob, dim=1)] = 1
                    m_single += m
                    
                    m_group = torch.zeros_like(m)
                    m_group[prob > 0.5] = 1
                    m_group += m
                    
                    pred_single = predictor(mask_layer(x, m_single))
                    pred_group = predictor(mask_layer(x, m_group))
                    
                    single_ce = nn.CrossEntropyLoss()
                    group_ce = nn.CrossEntropyLoss()
                    
                    # we use beta to calculate the total acuireing new features in m_group
                    if torch.sum(m_group - m) != 0:
                        loss = alpha * single_ce(pred_single, y) + (1 - alpha) * group_ce(pred_group, y) + beta * torch.sum(m_group - m)
                    else: 
                        loss = single_ce(pred_single, y)
                        stop = True
            ...
            with torch.no_grad():
                ...
                for x, y in val_loader:
                    ...
                    while not stop:
                        ...
                        pred_list.append(pred_single.cpu())
